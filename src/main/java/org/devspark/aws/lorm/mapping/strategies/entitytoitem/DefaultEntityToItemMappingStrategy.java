...
import org.devspark.aws.lorm.schema.AttributeConstraintType;
...
public class DefaultEntityToItemMappingStrategy implements EntityToItemMappingStrategy {

    protected final ReflectionSupport reflectionSupport;

    public DefaultEntityToItemMappingStrategy(ReflectionSupport reflectionSupport) {
	super();
	...
	for (SchemaValidationError schemaValidationError : validationErrors) {
	    ...
	    } else if (schemaValidationError.getErrorType()
		    .equals(SchemaValidationErrorType.GENERAL)) {
		throw new DataValidationException(
