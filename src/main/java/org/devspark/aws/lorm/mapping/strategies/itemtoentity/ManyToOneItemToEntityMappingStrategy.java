...
public class ManyToOneItemToEntityMappingStrategy implements ItemToEntityMappingStrategy {

    private final ReflectionSupport reflectionSupport;
    private final EntityManager entityManager;
    private final Log log = LogFactory.getLog(getClass());

    public ManyToOneItemToEntityMappingStrategy(ReflectionSupport reflectionSupport,
	...
	Repository<?> refRepository = entityManager.getRepository(field.getType());
	Object refInstance = refRepository.findOne(refId);
	if (refInstance != null) {
	    reflectionSupport.setValueOfField(field, entityInstance, refInstance);
	} else {
	    log.warn("Reference not found " + field.getType() + " with id: " + refId
